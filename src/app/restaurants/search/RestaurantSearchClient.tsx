...
export default function RestaurantSearchClient() {
  ...
  async function handleSearch(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const restaurants = await searchRestaurants(supabase, normalizedQuery)
          ...
          <section className="mt-8" aria-live="polite">
            ...
            {results.length > 0 ? (
                  ...
                  <li key={restaurant.id}>
                    <Link href={`/restaurants/${restaurant.id}`} className="block">
                      <Card interactive className="p-4">
                        ...
                        <p className="mt-3 text-sm font-medium text-terra">詳細を見る →</p>
                      </Card>
                    </Link>
                  </li>
                ))}
              ...
              <Card className="p-6 text-center">
                ...
                <Link
                  href={`/restaurants/new?name=${encodeURIComponent(query.trim())}`}
